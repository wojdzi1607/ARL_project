...
class Autonomous:
    ...
    def autonomous_flying(self, corners):
        ...
        for marker in corners:
            ...
            distance = np.sqrt(np.power(x_center, 2) + np.power(y_center, 2))

            if x_center < 0.45 and y_center < 0.45:
                # print('forward left')
                self.command.linear.x = distance
                self.command.linear.y = distance/4
            if x_center > 0.45 and y_center < 0.45:
                # print('forward right')
                self.command.linear.x = distance
                self.command.linear.y = -distance/4
            if x_center < 0.45 and y_center > 0.45:
                # print('backward left')
                self.command.linear.x = -distance/4
                self.command.linear.y = distance/4
            if x_center > 0.45 and y_center > 0.45:
